...
export default async function InventoryFlowersPage({ params }: Props) {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-tight">Flores</h1>
  ...
  const [strains, stocks, products, lotLinks] = await Promise.all([
    ...
    prisma.product.findMany({
      where: { tenantId: tenant.id, category: { in: ["flores", "plant_material"] }, status: "active" },
      orderBy: { name: "asc" },
    }),
